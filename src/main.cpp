...
bool ShargeState = true;   //* Если выключить то портится измирение напряжения Переменный буль состояния открытости-закрытости транзистора открывающего цепь заряда
...
// Печать результата
  ...
  Serial.print("  averageAmp: ");Serial.print(averageAmp);
  
  Serial.print("  amp: "); 
  Serial.print( float(13177 - averageAmp)/731 ,3); //13300 Примерное значение при нулевой нагрузке на ACS712. Чем больше нагрузка тем ниже напряжение приходит. Отнимаем разницу.Делим на подобраное число 
